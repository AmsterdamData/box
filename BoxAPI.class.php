<?php 
    ...
    class Box_API {
        ...
        public function get_code($error_msg = null) {
        ...
        public function get_file_details($file, $json = false, $opts = null) {
            if($opts){
                $url = $this->build_url("/files/$file",$opts);
            } else {
                $url = $this->build_url("/files/$file");
            }
            if($json){    
                return $this->get($url); //."&fields=tags"
            } else {                              
                return json_decode($this->get($url),true); //."&fields=tags"
            }
        }
        ...
        public function read_token($type = 'file', $json = false) {
            if($type == 'file' && file_exists('token.box') && filesize('token.box') > 0){
        ...
        private static function post($url, $params, $contentType = null) {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        ...
        private static function put($url, array $params = array(), $contentType = null) {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        ...
        //Toevoegingen JS, 28-10-2014
        public function upsert_metadata($file, array $metadata_to_add){
            $url = $this->build_url("/files/$file/metadata/properties");
            $metadata = json_decode($this->get($url),true);
            if(array_key_exists("\$type", $metadata)){
                $params = Array();
                foreach($metadata_to_add as $key => $value){
                    if(array_key_exists($key, $metadata)){
                        $params[] = Array("op" => "replace", "path" => "/". $key ,"value" => $value);
                    } else {
                        $params[] = Array("op" => "add", "path" => "/". $key ,"value" => $value);
                    }
                }
                $result = json_decode($this->put($url, $params, "application/json-patch+json"), true);
            } else {
                $params = $metadata_to_add;
                $url = $this->build_url("/files/$file/metadata/properties");
                $result = json_decode($this->post($url, json_encode($params), "application/json"), true);
            }
            return $result;
        }
